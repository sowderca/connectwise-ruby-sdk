...
module ConnectWise
  ...
  class ServiceLocation
    ...
    def list_invalid_properties
      ...
      if @where.nil?
        invalid_properties.push("invalid value for 'where', where cannot be nil.")
      end
    ...
    # @param [Object] where Object to be assigned
    def where=(where)
      validator = EnumAttributeValidator.new('String', ["OnSite", "Remote", "InHouse"])
      unless validator.valid?(where)
        fail ArgumentError, "invalid value for 'where', must be one of #{validator.allowable_values}."
      end
      @where = where
    end
    ...
    def hash
      [id, name, where, default_flag, _info].hash
    end
